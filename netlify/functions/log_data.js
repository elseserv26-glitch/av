...
async function getOrCreateTopicForIP(ipAddress) {
  ...
  try {
    ...
    if (error.message && error.message.includes('already exists')) {
      ...
      const retryTopicId = await findExistingTopicForIP(ipAddress);
      if (retryTopicId) {
        ipToTopicMap.set(ipAddress, retryTopicId);
        return retryTopicId;
      }
    }
...
/**
 * Sender en melding til Telegram
 */
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Hent IP-adresse fra headers (Netlify setter x-forwarded-for og client-ip)
    const forwardedFor = event.headers['x-forwarded-for'];
    const ip_adresse = forwardedFor
      ? forwardedFor.split(',')[0].trim()
      : event.headers['client-ip'] || event.headers['x-real-ip'] || 'Ukjent IP';
